#!/usr/bin/env python3
"""LeadFlow v2 generator — writes style.css and app.js"""
import os
...
# ═══════════════════════════════════════════════════════════════════════════
css = r"""
/* LeadFlow Platform v2 — Premium UI  •  Generated */
...
body{background:var(--bg);color:var(--text);font-family:var(--font-b);line-height:1.6;overflow:hidden;height:100vh}
...
/* ── Noise Overlay ── */
.noise-overlay{position:fixed;inset:0;z-index:1;pointer-events:none;opacity:.035;
...
.login-box{background:var(--glass);border:1px solid var(--glass-border);border-radius:24px;padding:48px 40px;backdrop-filter:blur(30px);width:100%;max-width:420px;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,.4)}
...
/* ═══ SIDEBAR ═══ */
.sidebar{background:var(--bg2);border-right:1px solid var(--glass-border);display:flex;flex-direction:column;align-items:center;padding:10px 0;gap:2px;overflow-y:auto;z-index:10}
...
.hamburger{display:none;position:absolute;top:10px;left:10px;z-index:20;background:none;border:none;color:var(--text);font-size:1.3rem}
...
/* ═══ PAGE TRANSITIONS ═══ */
.content-scroll.transitioning{animation:pageFadeIn .3s ease}
...
.country-tab{padding:7px 16px;border-radius:100px;border:1px solid var(--glass-border);background:var(--glass);color:var(--text2);font-size:.8rem;font-weight:600;cursor:pointer;transition:all .25s;display:flex;align-items:center;gap:5px;position:relative;overflow:hidden}
...
.overview-card:hover{transform:translateY(-2px);box-shadow:0 8px 30px rgba(0,0,0,.2);border-color:rgba(255,255,255,.12)}
...
.bento-card{background:var(--glass);border:1px solid var(--glass-border);border-radius:var(--radius-xs);padding:14px;transition:all .3s;contain:layout style}
.bento-card:hover{border-color:rgba(255,255,255,.1)}
...
.contact-item.s-pending{border-left-color:#888}
.contact-item.s-progress{border-left-color:var(--info)}
.contact-item.s-done{border-left-color:var(--success)}
.contact-item.s-lost{border-left-color:var(--danger)}
...
.ci-actions{display:flex;gap:4px;margin-top:6px;flex-wrap:wrap;align-items:center}
.ci-action-btn{width:28px;height:28px;border-radius:6px;border:1px solid var(--glass-border);background:var(--glass);color:var(--text2);font-size:.7rem;display:flex;align-items:center;justify-content:center;transition:all .2s;cursor:pointer}
.ci-action-btn:hover{border-color:var(--accent);color:var(--accent)}
.ci-action-btn.done-btn:hover{border-color:var(--success);color:var(--success)}
.ci-action-btn.reset-btn:hover{border-color:var(--warn);color:var(--warn)}
.ci-action-btn.comment-btn:hover{border-color:var(--info);color:var(--info)}
.ci-label-btn{padding:3px 8px;border-radius:100px;border:1px solid var(--glass-border);background:var(--glass);color:var(--text2);font-size:.6rem;cursor:pointer;transition:all .2s;white-space:nowrap}
.ci-label-btn.active-green{border-color:var(--success);color:var(--success);background:rgba(52,211,153,.08)}
.ci-label-btn.active-red{border-color:var(--danger);color:var(--danger);background:rgba(255,62,108,.08)}
.ci-label-btn.active-blue{border-color:var(--info);color:var(--info);background:rgba(56,189,248,.08)}
.ci-comment-section{width:100%;margin-top:6px;display:none}
.ci-comment-section.open{display:block}
.ci-comment-textarea{width:100%;min-height:50px;padding:8px;border-radius:6px;border:1px solid var(--glass-border);background:rgba(255,255,255,.03);color:var(--text);font-size:.75rem;font-family:var(--font-b);resize:vertical;outline:none;transition:border .3s}
.ci-comment-textarea:focus{border-color:rgba(185,255,102,.4)}

/* Checkmark animation */
.checkmark-anim{display:inline-flex;width:16px;height:16px;animation:checkBounce .4s ease}
...
.btn-danger{padding:8px 18px;border:1px solid rgba(255,62,108,.3);border-radius:7px;background:rgba(255,62,108,.06);color:var(--danger);font-size:.82rem;transition:all .2s}
...
/* ═══ SIDEBAR RIGHT (AI) ═══ */
.sidebar-right{background:var(--bg2);border-left:1px solid var(--glass-border);display:flex;flex-direction:column;overflow:hidden}
...
.ai-close{display:none;width:24px;height:24px;border-radius:6px;border:none;background:rgba(255,255,255,.05);color:var(--text2);font-size:.75rem;cursor:pointer;align-items:center;justify-content:center}
...
.modal-box{background:var(--bg2);border:1px solid var(--glass-border);border-radius:var(--radius);width:100%;max-width:560px;max-height:85vh;overflow-y:auto;padding:0;position:relative;animation:slideUp .3s ease}
@keyframes slideUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
.modal-close{position:absolute;top:14px;right:14px;width:32px;height:32px;border-radius:8px;border:1px solid var(--glass-border);background:var(--glass);color:var(--text2);font-size:.9rem;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all .2s;z-index:2}
.modal-close:hover{border-color:var(--danger);color:var(--danger)}
.modal-header{padding:24px 24px 0}
.modal-company{font-family:var(--font-h);font-size:1.4rem;font-weight:700;background:var(--grad);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;margin-bottom:4px}
.modal-meta{display:flex;flex-wrap:wrap;gap:8px;margin-top:8px}
.modal-meta-item{font-size:.78rem;color:var(--text2