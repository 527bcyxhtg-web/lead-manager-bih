#!/usr/bin/env python3
"""Write all LeadFlow v2 files."""
import os, json

BASE = "/Users/maki/.qwenpaw/workspaces/cloud-orchestrator/lead-manager-repo/v2"

# ============================================================
# style.css  (Part 1 of 1)
# ============================================================
CSS = r"""/* LeadFlow Platform v2 — Complete Styles */
...
/* ── Theme Palettes ── */
[data-theme="dark"]{--bg:#050508;--bg2:#0a0a12;--surface:rgba(255,255,255,.03);--glass:rgba(255,255,255,.04);--glass-border:rgba(255,255,255,.08);--accent:#b9ff66;--accent2:#39ff14;--accent3:#00ff88;--grad:linear-gradient(135deg,#b9ff66,#39ff14);--text:#e8e8f0;--text2:#8888a0;--text3:#555570;--danger:#ff3e6c;--warn:#fbbf24;--info:#38bdf8;--success:#34d399}
[data-theme="light"]{--bg:#f5f5f7;--bg2:#e8e8ec;--surface:rgba(0,0,0,.03);--glass:rgba(255,255,255,.75);--glass-border:rgba(0,0,0,.08);--accent:#16a34a;--accent2:#15803d;--accent3:#166534;--grad:linear-gradient(135deg,#16a34a,#15803d);--text:#1a1a2e;--text2:#555;--text3:#999;--danger:#dc2626;--warn:#d97706;--info:#2563eb;--success:#16a34a}
[data-theme="ocean"]{--bg:#031b26;--bg2:#062a3e;--surface:rgba(255,255,255,.03);--glass:rgba(255,255,255,.04);--glass-border:rgba(56,189,248,.12);--accent:#38bdf8;--accent2:#0ea5e9;--accent3:#06b6d4;--grad:linear-gradient(135deg,#38bdf8,#06b6d4);--text:#e0f2fe;--text2:#7dd3fc;--text3:#38bdf8;--danger:#f43f5e;--warn:#fbbf24;--info:#38bdf8;--success:#34d399}
[data-theme="sunset"]{--bg:#1a0a05;--bg2:#2d1510;--surface:rgba(255,255,255,.03);--glass:rgba(255,255,255,.04);--glass-border:rgba(251,146,60,.12);--accent:#f97316;--accent2:#ea580c;--accent3:#fb923c;--grad:linear-gradient(135deg,#f97316,#f59e0b);--text:#fef3c7;--text2:#fdba74;--text3:#f97316;--danger:#ef4444;--warn:#eab308;--info:#38bdf8;--success:#34d399}
[data-theme="neon"]{--bg:#020c0b;--bg2:#041a18;--surface:rgba(57,255,20,.02);--glass:rgba(57,255,20,.03);--glass-border:rgba(57,255,20,.1);--accent:#39ff14;--accent2:#22c55e;--accent3:#4ade80;--grad:linear-gradient(135deg,#39ff14,#22c55e);--text:#dcfce7;--text2:#86efac;--text3:#22c55e;--danger:#ef4444;--warn:#fbbf24;--info:#38bdf8;--success:#39ff14}
[data-theme="purple"]{--bg:#0a0514;--bg2:#130a24;--surface:rgba(139,92,246,.03);--glass:rgba(139,92,246,.04);--glass-border:rgba(139,92,246,.12);--accent:#a78bfa;--accent2:#8b5cf6;--accent3:#c4b5fd;--grad:linear-gradient(135deg,#8b5cf6,#a78bfa);--text:#ede9fe;--text2:#c4b5fd;--text3:#8b5cf6;--danger:#f43f5e;--warn:#fbbf24;--info:#38bdf8;--success:#34d399}
...
/* ── Noise Overlay ── */
.noise-overlay{position:fixed;inset:0;z-index:1;pointer-events:none;opacity:.035;background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E")}
...
.ripple::after{content:'';position:absolute;border-radius:50%;background:rgba(255,255,255,.15);width:100px;height:100px;margin-top:-50px;margin-left:-50px;top:50%;left:50%;transform:scale(0);opacity:1;transition:transform .5s,opacity .5s}
...
.login-box{background:var(--glass);border:1px solid var(--glass-border);border-radius:24px;padding:48px 40px;backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);width:100%;max-width:420px;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,.4);animation:floatIn .6s ease}
...
.login-av:hover{transform:scale(1.08);box-shadow:0 4px 20px rgba(255,255,255,.1)}
.login-av.sel{border-color:var(--accent);box-shadow:0 0 20px rgba(185,255,102,.25)}
...
.sidebar-icon{width:40px;height:40px;border-radius:10px;border:none;background:transparent;color:var(--text3);font-size:1.15rem;display:flex;align-items:center;justify-content:center;transition:all .2s;position:relative}
...
.sidebar-icon.active::before{content:'';position:absolute;left:-10px;top:50%;transform:translateY(-50%);width:3px;height:20px;border-radius:0 3px 3px 0;background:var(--accent)}
...
/* ── Hamburger ── */
.hamburger{display:none;border:none;background:transparent;color:var(--text);font-size:1.3rem;padding:4px 8px}

/* ── Topbar ── */
.main-area{display:flex;flex-direction:column;overflow:hidden}
.topbar{display:flex;align-items:center;justify-content:space-between;padding:10px 20px;border-bottom:1px solid var(--glass-border);background:rgba(255,255,255,.01);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);flex-shrink:0;z-index:5;transition:background .3s}
...
.overview-card{background:var(--glass);border:1px solid var(--glass-border);border-radius:var(--radius-sm);padding:18px;position:relative;overflow:hidden;transition:all .3s}
...
.overview-card:hover{transform:translateY(-3px);box-shadow:0 8px 30px rgba(0,0,0,.3);border-color:rgba(255,255,255,.12)}
...
.bento-card:hover{border-color:rgba(255,255,255,.12);transform:translateY(-1px)}
...
.contact-item{display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--glass);border:1px solid var(--glass-border);border-radius:var(--radius-xs);transition:all .25s;cursor:pointer;position:relative}
.contact-item::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;border-radius:3px 0 0 3px;transition:background .3s}
.contact-item[data-status="pending"]::before{background:#555}
...
.contact-item[data-status="lost_sale"]::before{background:var(--danger)}
...
.ci-actions{display:flex;gap:4px;opacity:0;transition:opacity .2s}
.contact-item:hover .ci-actions{opacity:1}
.ci-act-btn{width:28px;height:28px;border-radius:6px;border:1px solid var(--glass-border);background:var(--glass);color:var(--text2);font-size:.75rem;display:flex;align-items:center;justify-content:center;transition:all .2s}
.ci-act-btn:hover{border-color:var(--accent);color:var(--accent)}
.ci-act-btn.done-btn:hover{border-color:var(--success);color:var(--success)}
.ci-act-btn.reset-btn:hover{border-color:var(--warn);color:var(--warn)}
.ci-act-btn.sale-btn{font-size:.65rem;width:auto;padding:0 6px}
.ci-act-btn.sale-btn.active-sale{background:rgba(52,211,153,.15);border-color:var(--success);color:var(--success)}
.ci-act-btn.sale-btn.active-lost{background:rgba(255,62,108,.15);border-color:var(--danger);color:var(--danger)}
.ci-act-btn.demo-btn{font-size:.65rem;width:auto;padding:0 6px}
.ci-act-btn.demo-btn.active-demo{background:rgba(56,189,248,.15);border-color:var(--info);color:var(--info)}

/* Checkmark animation */
@keyframes drawCheck{0%{stroke-dashoffset:24}100%{stroke-dashoffset:0}}
.check-anim{display:inline-block}
.check-anim svg{width:14px;height:14px}
.check-anim svg path{stroke-dasharray:24;stroke-dashoffset:24;animation:drawCheck .4s ease forwards}
...
/* ══════════════════════════════════════
   ...
   ══════════════════════════════════════ */
.sidebar-right{background:var(--bg2);border-left:1px solid var(--glass-border);display:flex;flex-direction:column;overflow:hidden;transition:background .3s}
...
.ai-close{border:none;background:transparent;color:var(--text3);font-size:1rem;padding:4px;border-radius:4px;display:none}
...
.script-card .sc-d{font-size:.72rem